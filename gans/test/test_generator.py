...
class generator_test(unittest.TestCase):
    ...
    def test_generator_block(self):
        ...
        gen = get_generator_block(in_features, out_features)

        self.assertTrue(len(gen)==3, "block number is not 3")
        self.assertIsInstance(gen[0], nn.Linear, "1st block is not Linear")
        self.assertIsInstance(gen[1], nn.BatchNorm1d, "2nd block is not BatchNorm")
        self.assertIsInstance(gen[2], nn.ReLU, "3rd block is not ReLU")
        
        test_input = torch.randn(in_features, out_features)
        test_output = get_generator_block(test_input)
